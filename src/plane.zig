...
pub fn intersect(ts: *std.ArrayList(int.Intersection), p: Plane, r: ray.Ray) !void {
    ...
    const r_prime = ray.transform(r, mat.inverse(p.common_attrs.transform));
    ...
    try ts.append(int.Intersection{
        ...
        .normal = normalAt(p, ray.position(r, t)),
    });
}

pub fn normalAt(plane: Plane, world_point: tup.Point) tup.Vector {
    ...
    var n = mat.mul(mat.transpose(mat.inverse(plane.common_attrs.transform)), tup.vector(0, 1, 0));
...
test "the normal of a plane is constant everywhere" {
    const p = Plane{};
    const n1 = normalAt(p, tup.point(0, 0, 0));
    const n2 = normalAt(p, tup.point(10, 0, -10));
    const n3 = normalAt(p, tup.point(-5, 0, 150));
...
test "a ray intersecting a plane from above" {
    ...
    try expectEqual(xs.items[0].shape_attrs, p.common_attrs);
}
